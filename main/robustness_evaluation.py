...
async def run_parallel_generation_vllm(
    ...
    try:
        for model in models:
            if "openai/" in model:
                clients[model] = AsyncOpenAI(api_key=OPENAI_API_KEY, base_url="https://api.openai.com/v1")
            else:
                clients[model] = AsyncOpenAI(api_key='EMPTY', base_url=MODEL_SERVER_MAPPING[model])
